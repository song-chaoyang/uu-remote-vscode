...
右键「详情」打开 Webview 面板(跟随 VSCode 主题):展示 ID、平台、在线 / 连接状态等,并内嵌操作按钮(串流连接、打开远程终端、启动程序、上传/下载文件、开关机、复制 ID),操作后自动刷新状态。

### 文件传输

右键设备 →「上传文件到设备 / 从设备下载文件」:走 VSCode 原生文件选择/保存对话框,自动取远程主目录作为默认路径(上传 ≤512KB、下载 ≤256KB;更大文件请使用 UU远程主程序的文件传输)。

### 远程启动程序

右键设备 →「启动程序」:一键在远程打开记事本、任务管理器、计算器、资源管理器、cmd,或输入任意程序路径(Start-Process);AI 工具 `uu_remote_launch` 同样可用。

### 无线副屏

命令面板「UU远程: 无线副屏」或右键设备 → 打开引导面板(功能特性 / 使用步骤 / 布局方向提示),一键启动 UU远程主程序直达副屏入口,官方介绍与下载页直达。(副屏连接由 UU远程主程序完成,插件负责引导与入口)
